...
function TodoForm({ props }: { props: TodoFormType }) {
    ...
    return (
        <form className={todoStyle.todoForm} onSubmit={(formElm: ChangeEvent<HTMLFormElement>) => {
            ...
            {/* 予約室 */}
            <TodoFormItemRoom rooms={rooms} todoItems={todoItems} setTodoItems={setTodoItems} roomRef={roomRef} setValidationTxt={setValidationTxt} validationTxt={validationTxt} />

            {/* タイムテーブル（スケジュール）*/}
            <TodoFormItemTimeSchedule todoItems={todoItems} setTodoItems={setTodoItems} setValidationTxt={setValidationTxt} validationTxt={validationTxt} />
